    ...
    H: Digest,
{
    ...
    let r_known = g * (bob_k_b * (bob_h + alice_k_a_pr));
    assert_eq!(r_known, bob_r);
    ...
    let alice_gamma_2 = pk * alice_t_1_a - g * alice_t2_a;
    let alice_hg_2 = utils::bytes_to_scalar::<F>(H::digest(alice_gamma_2.to_bytes()).as_ref());
    let alice_sig_a = m * alice_t_1_a + alice_r_x * alice_t2_a;
    ...
    let bob_theta = bob_t_1_b - bob_phi * bob_k_b.invert().unwrap();
    let bob_sig_b = m * bob_theta + bob_r_x * bob_t2_b;
    let bob_gamma_2 = g * bob_t2_b - pk * bob_theta;
    let bob_hg_2 = utils::bytes_to_scalar::<F>(H::digest(bob_gamma_2.to_bytes()).as_ref());
    assert_eq!(alice_gamma_2, bob_gamma_2);
    let bob_sig_a = bob_eta_sig - bob_hg_2;
